...
Let us look closer to the data resulting from the [Data Wrangling](https://github.com/tajataja/IODS-project/blob/master/data/create_learning2014.R). 
<br /><br /> 
...
```
<br /> 
<br />So here we have a large plot matrix of all the 7 variables. Red colour indicates female answerers and blue men answerers. It is worth noticing that there are nearly twice as many female answerers as men and that ajority of the students are 20 to 30 years old.
...
<br /> 
<br /> As expected the value of multiple R-squared decreases, but only slightly. The new values _(see below)_ are; 
...
In addition to R-squared values we may also look at, for example, the p-value. When all the variables are included, the p-value is 5.217e-08 _(above)_ and after deep learning has excluded the p-value is 2.041e-08 _(below)_, ergo the significance has strengthen. Although, the significance is notable in both cases.
...
<br /> Next we will draw three diagnostic plots out of the multiple regression, which explanatory variables are above-mentioned attitude and surface learning. The chosen diagnostic plots are _Residuals vs Fitted, Normal QQ-plot_ and _Residuals vs Leverage_ _(table below, 1, 2, 5)_. 
<br /><br />
